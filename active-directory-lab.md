...
- **AD administration** — user and group management, OU design, Group Policy, NTFS share permissions, domain joins
- **Security architecture** — Microsoft tiered model (Tier 0/1/2), least privilege, separation of duties, and paired admin accounts to contain lateral movement
- **Automation** — idempotent PowerShell that builds and tears down the whole environment safely
- **Standards literacy** — NIST SP 800-207 (zero trust) and SP 800-63B r4 (modern authentication), plus Microsoft's Enterprise Access Model
- **Support practice** — realistic Level 1–3 ticket scenarios
- **SOC practice** — logon monitoring, detecting admin-account misuse, and building incident timelines
...
| Domain controller | `DC01` — `192.168.4.100` (Tier 0) |
| File server | `FS01` — `192.168.4.101` (Tier 1) |
