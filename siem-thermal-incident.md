...
- **Root-cause analysis under a misleading symptom.** The presenting error pointed at a database; the actual fault was three layers down in hardware. I resisted the obvious fix (database repair) that would have wasted time and risked making things worse.
- **Reading the stack top-to-bottom.** Application error → container config → filesystem logs → block-device/NVMe kernel messages → SMART health, plus the proxy → backend → datastore request path for the secondary issue.
- **Hypothesis-driven troubleshooting.** Each theory (swap, worn drive, database corruption, nginx, incomplete upgrade, authorisation) was tested against evidence and either confirmed or ruled out before acting — not guessed at.
- **Knowing when it's hardware, not software.** Recognising the PCIe de-enumeration signature (`CSTS=0xffffffff`, capacity → 0) and the SMART thermal counters as definitive.
- **Safe recovery discipline.** No `xfs_repair` on a live or possibly-failing volume; cold cycle over warm reboot; data integrity prioritised; verifying drive health before trusting it again.
- **Using authoritative sources.** Confirming the run-as fix against the vendor's own issue tracker rather than improvising.
- **Translating diagnosis into prevention.** Identifying a concrete, permanent hardware remediation rather than repeatedly recovering from the same outage.
